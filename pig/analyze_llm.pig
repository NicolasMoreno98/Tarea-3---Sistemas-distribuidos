-- Script de Apache Pig para analizar respuestas del LLM
-- Análisis de frecuencia de palabras con limpieza y filtrado

-- Cargar datos desde HDFS
llm_data = LOAD '/input/llm_answers.txt' USING PigStorage('\t') AS (answer:chararray);

-- Tokenización: separar en palabras
tokens = FOREACH llm_data GENERATE FLATTEN(TOKENIZE(LOWER(answer))) AS word;
...
STORE top_100 INTO '/output/llm_top100' USING PigStorage('\t');
